...
The project ships stable and canary releases; nothing here is a substitute for
reviewing Cloudflare, each upstream MCP server, and the exact signed release
before deployment.
...
- Signed stable and canary releases are available; support is best-effort
  under the [support policy](../SUPPORT.md), not a production SLA. Review the
  exact
