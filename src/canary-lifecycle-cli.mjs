...
} from './canary-lifecycle-command.mjs';
import { CanaryLifecycleError } from './canary-runner.mjs';
import { STALE_LOCK_RECOVERY_CONFIRMATION } from './receipt-store.mjs';

const USAGE = `Usage:
  node src/canary-lifecycle-cli.mjs preview --account-id <id> --zone-id <id> --hostname <ankka-canary.example.com> --synthetic-mcp-url <https-url>/mcp --receipt <path> [--json]
  node src/canary-lifecycle-cli.mjs run --account-id <id> --zone-id <id> --hostname <ankka-canary.example.com> --synthetic-mcp-url <https-url>/mcp --receipt <path> --approve <lifecycle-id> --confirm-disposable-target <target-id> [--json]
  node src/canary-lifecycle-cli.mjs lock inspect --receipt <path> --store <receipt|cleanup> [--json]
  node src/canary-lifecycle-cli.mjs lock recover --receipt <path> --store <receipt|cleanup> --lock-id <id> --confirm ${STALE_LOCK_RECOVERY_CONFIRMATION} [--json]
...
try {
  ...
  if (invocation.help) {
    ...
    const result = await executeCanaryLifecycleCommand(invocation.lifecycle, {
      ...(invocation.lifecycle.json
        ? {}
        : {
            onProgress(event) {
...
function parse(args) {
  ...
  if (!['preview', 'run'].includes(args[0])) {
  ...
  const invocation = {
    ...
    zoneId: undefined,
    hostname: undefined,
    syntheticMcpUrl: undefined,
    receiptPath: undefined,
    approvalId: undefined,
    targetConfirmationId: undefined,
    json: false,
  };
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    if (!property) throw new CliInputError('The canary lifecycle command received an unknown option.');
  ...
  if (invocation.mode === 'preview') {
    delete invocation.approvalId;
    delete invocation.targetConfirmationId;
  }
  return { command: 'lifecycle', lifecycle: invocation };
}

function parseLock(args) {
  ...
  for (let index = 1; index < args.length; index += 1) {
    ...
    const property = options.get(argument);
    if (!property) throw new CliInputError('The canary lock command received an unknown option.');
    const value = args[index + 1];
    if (!value || value.startsWith('-')) {
      throw new CliInputError(`${argument} requires a value.`);
    }
  ...
  for (const [option, property] of [
    ...
    if (invocation[property] === undefined) throw new CliInputError(`${option} is required.`);
  }
  if (!['receipt', 'cleanup'].includes(invocation.store)) {
  ...
  if (invocation.operation === 'inspect') {
    ...
    delete invocation.lockId;
    delete invocation.confirmation;
  } else {
