...
function bindings() {
  const values = {
    ...
    ANKKA_INSTALL_ID: `acg-${'d'.repeat(24)}`,
...
async function updateTransportFixture(
  ...
  const runtime = await sourceActionRuntimeFixture(runtimeInput);
...
describe('V1 runtime credential boundary', () => {
  it('rejects every undeclared standing credential before any provider write', async () => {
