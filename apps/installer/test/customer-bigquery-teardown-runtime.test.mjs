...
async function fixture(run, { stopAfter, knownPending = false, portalPending = false, lostDelete = -1 } = {}) {
  ...
  const bridge = await bridgeFixture({ fixtureContext: context, lostDelete, journalStorage: storage });
  const instances = new Map();
  const network = async (request) => {
    const url = new URL(request.url);
    if (url.hostname === bridge.record.hostname) {
      return new Response(null, { status: 401, headers: {
        'WWW-Authenticate': `Bearer resource_metadata="https://${bridge.record.hostname}/.well-known/oauth-protected-resource"`,
      } });
    }
    ...
    if (url.pathname.startsWith(`/client/v4/accounts/${ACCOUNT_ID}/workers/`) || url.pathname.endsWith('/access/apps/bridge-app')) {
      return bridge.fetch(request.url, { method: request.method, headers: request.headers });
    }
  ...
  const prepared = await setup.prepare(request('/api/bigquery', { schemaVersion: 1, revision: storage.snapshot(SOURCES).revision,
    label: 'BigQuery', configuration: bridge.record.configuration, readOnlyConfirmed: true }), 'admin@example.com', false);
  expect(prepared.status).toBe(200);
  const claim = JSON.parse(Buffer.from(new URL((await prepared.json()).handoffUrl).hash.slice(1), 'base64url').toString());
  const generated = await storage.get(bridge.key);
  await storage.put(bridge.key, { ...bridge.record, actionId: claim.actionId, sourceHash: generated.sourceHash });
  ...
  async function teardown(seed = 't') {
    ...
    return { prepared, async send(command, requestId = 'u'.repeat(22)) {
      ...
      return signed(`/teardown-actions/${command}-current`, body, key, 'x-ankka-teardown-action-signature');
    } };
  }
  await withProviderFetch(network, async () => {
    ...
    await run({ ...gateway, storage, bridge, runtime, teardown, sourceAction: action });
  });
}
