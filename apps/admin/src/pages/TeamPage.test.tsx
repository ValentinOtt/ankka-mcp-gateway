...
import { GatewayApiError, type GatewayAdminApi, type GatewayStatus, type ManagedSources, type PreparedAction, type RuntimeUpdate, type Team, type TeamAction } from '../api'
...
describe('TeamPage', () => {
  ...
  it('keeps existing-source permission controls usable while source addition is paused', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Review in Cloudflare' })).toBeEnabled()
  ...
  it('submits only the reviewed proposal and never optimistically changes saved access', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Review in Cloudflare' }))
    expect(prepareTeamAction).toHaveBeenCalledWith(7, [
  ...
  it('keeps a recovery proposal read-only and resumes its exact recorded membership', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Resume change in Cloudflare' }))
  ...
  it('does not permit a recovery action when its recorded proposal cannot be retrieved', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Resume change in Cloudflare' })).toBeDisabled()
    expect(client.prepareTeamAction).not.toHaveBeenCalled()
  })
  ...
  it('rejects an untrusted authorization destination without claiming the change was applied', async () => {
    const user = userEvent.setup()
    renderTeam(api({ prepareTeamAction: vi.fn(async (): Promise<PreparedAction> => ({ schemaVersion: 1, actionId, status: 'authorization_required', expiresAt, handoffUrl: `https://untrusted.example.com/manage#${'a'.repeat(40)}` })) }))
    const person = await screen.findByRole('group', { name: 'analyst@example.com' })
    await user.click(within(person).getByRole('checkbox'))
    await user.click(screen.getByRole('button', { name: 'Review in Cloudflare' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('The authorization link could not be verified.')
    expect(screen.queryByText(/last recorded team access change was applied and verified/)).not.toBeInTheDocument()
  })
  ...
  it('locks uncertain preparation failures until refresh and never exposes raw exception details', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Review in Cloudflare' })).toBeDisabled()
  ...
  it('does not claim cancellation when the recorded action started applying before cancellation', async () => {
    ...
    expect(screen.getByRole('button', { name: 'Continue in Cloudflare' })).toBeDisabled()
  ...
  it('shows a safe unavailable state when team loading fails', async () => {
    ...
    expect(screen.queryByText('private provider detail')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Review in Cloudflare' })).not.toBeInTheDocument()
  })
  ...
  it('keeps recovery resume disabled while the release is awaiting approval', async () => {
    ...
    const resume = await screen.findByRole('button', { name: 'Resume change in Cloudflare' })
    expect(resume).toBeDisabled()
    await user.click(resume)
    expect(client.prepareTeamAction).not.toHaveBeenCalled()
  })

  it('explicitly labels the local synthetic preview and retains its simulated authorization on this page', async () => {
    vi.stubEnv('VITE_GATEWAY_UI_PREVIEW', '1')
    const user = userEvent.setup()
    const pendingAction: TeamAction = { schemaVersion: 1, actionId, status: 'authorization_required', expiresAt, failureCode: null, canCancel: true }
    renderTeam(api({
      prepareTeamAction: vi.fn(async (): Promise<PreparedAction> => ({ schemaVersion: 1, actionId, status: 'authorization_required', expiresAt, handoffUrl: `https://deploy.ankka.ai/manage#${'a'.repeat(40)}` })),
      getTeamAction: vi.fn(async () => pendingAction),
    }))
