...
const authoredRecipes: readonly NativeConnectorRecipe[] = [
  ...
  },
  {
    ...
    upstreamControls: ['Use a dedicated identity with dataset-scoped Data Viewer, query-project Job User, and MCP Tool User access.', 'Apply Google’s deny policy for non-read-only MCP tools; exclude execute_sql and inherited broad permissions.', 'Set an approved query budget; a SQL LIMIT does not bound bytes scanned.'],
